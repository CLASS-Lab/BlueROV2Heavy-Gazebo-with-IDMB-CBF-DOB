#include "new_mpc_cbf_dob/new_mpc_cbf_dob_node.h"

namespace new_mpc_cbf_dob
{
    NewMpcCbfDobNode::NewMpcCbfDobNode(ros::NodeHandle &nh, double tc) :  nh_("~"),
                                                                    ...
                                                                    z_hat({0,0,0,0,0,0})
    {
        ...
        //   slice does not include the stop!
            ...
            casadi::SX nu_dot =
                mtimes(inv(M),
                       tau(all, i)+ tau_d - mtimes((C_RB + C_A), sym_nu) - mtimes((D_linear + D_nonlinear), sym_nu) - g_eta);
            ...
            for (int j = 0; j < N_OB; j++)
            {
                ...
                constraints = vertcat(
                    ...
                    Lf2h + mtimes(Lglfh, (tau(all, i)+tau_d)) + k_alpha(0) * h_fcn + k_alpha(1) * Lfh -(omega_1*omega_1)/(2*v1*beta)- beta *casadi::SX::mtimes(Lglfh,Lglfh.T())/(4*alpha-2*v1+2*p2)
        ...
        br2_state_sub = nh.subscribe("/bluerov2/pose_gt", 10, &NewMpcCbfDobNode::pose_gt_cb, this);
    ...
    void NewMpcCbfDobNode::pose_gt_cb(const nav_msgs::Odometry &msg)
    {
        // update std_eta
    ...
    void NewMpcCbfDobNode::control_pub(void)
    {
    ...
    void NewMpcCbfDobNode::run(void)
    {
    ...
    void NewMpcCbfDobNode::dob_euler(double dt)
    {
        Eigen::VectorXd dz(6), 
    ...
    Eigen::VectorXd NewMpcCbfDobNode::dob_state_equation(Eigen::VectorXd d_hat_tmp)
    {
...
{
    int control_rate = 10;
    ros::init(argc, argv, "new_mpc_cbf_dob_node");
    ros::NodeHandle nh;
    new_mpc_cbf_dob::NewMpcCbfDobNode new_mpc_cbf_dob_node(nh, 1. / control_rate);
    ros::Rate rate(control_rate);
    ROS_INFO_ONCE("**********************new_mpc_cbf_dob_node_start************************");
    while (ros::ok())
    {
        ros::spinOnce();
        new_mpc_cbf_dob_node.run();
        rate.sleep();
    }
    return 0;
}