#ifndef DYN_MB_MPC_CBF_DOB_INCLUDE_NEW_MPC_CBF_DOB_NODE_H
#define DYN_MB_MPC_CBF_DOB_INCLUDE_NEW_MPC_CBF_DOB_NODE_H
#endif
...
{
    class DynMbMpcCbfDobNode
    {
    public:
        DynMbMpcCbfDobNode(ros::NodeHandle &nh, double tc);
        void run(void);
    ...
    private:
        ...
        // this is for move blocking
        std::vector<int> blocking_indexes;
        int n_block;
        ...
        ros::Subscriber factor_sub;
        ros::Subscriber dyn_obstacles_sub;
        ...
        double obstacles_std[N_OB][8] =
            {{1, 33, 11, -95, 0, 0, 0, 1.2 + 0.3},
             ...
             {0, 28, 10, -95, 0, 0, 0, 1 + 0.3},
             {0, 30.9, 8, -95, 0, 0, 0, 1 + 0.3},
             {1, 25, 8, -95, 0, 0, 0, 1.2 + 0.3}};
        double obstacles[N_OB][8] =
            {{1, 33, -11, 95, 0, 0, 0, 1.5},
             {0, 24, -13, 95, 0, 0, 0, 1.3},
             {1, 30.5, -14, 95, 0, 0, 0, 2.9},
             {0, 28, -10, 95, 0, 0, 0, 1.3},
             {0, 30.9, -8, 95, 0, 0, 0, 1.3},
             {1, 25, -8, 95, 0, 0, 0, 1.5}};
        /************************************************** */
        // this is for the factor of the dynamic obstacle used in CBF
        std::vector<double> factor = std::vector<double>(N_DYN_OB,0.0);
        std::vector<std::vector<double>> dyn_obstacles = std::vector<std::vector<double>>(N_DYN_OB, std::vector<double>(8, 0.0));
        ...
        void factor_cb(const std_msgs::Float64MultiArray &msg);
        void dyn_obstacles_cb(const gazebo_msgs::ModelStates &msg);
    };
